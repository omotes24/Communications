...
管理画面は `/admin` です。管理者以外にはページ・APIとも一律404を返すため、機能の存在を判別できません。管理者アカウントはメールアドレスではなく、変更されにくい Supabase Auth の user UUID で指定します。
...
- 行動分析は既定で無効です。有効化前にプライバシー表示、保持期間、同意方針を確定してください。
