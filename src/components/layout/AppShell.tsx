...
export function AppShell({
  ...
  const [intent, setIntent] = useState<{ from: string; to: string } | null>(
    null,
  );
  ...
  return (
    <div
      className={cn(
        "relative isolate min-h-screen overflow-x-clip pb-28",
        isDark ? "bg-[#0d0d0f] text-white" : "bg-[#f5f5f7] text-[#1d1d1f]",
      )}
    >
      <ProductAnalytics />
      ...
      <header className="app-topbar sticky top-0 z-30 border-b">
        <div className="mx-auto flex h-16 max-w-[1180px] items-center justify-between px-4 sm:px-8">
          <Link
            href="/"
            className="text-[17px] font-extrabold tracking-[-0.35px] transition-opacity hover:opacity-70"
          ...
          <div className="flex items-center gap-2">
            {canCollapse ? (
              <button
                ...
                className="rounded-full bg-[var(--accent-light)] px-3 py-2 text-xs font-bold text-[var(--accent)]"
              >
      ...
      {!collapsed ? (
        <nav
          aria-label="主要画面"
          className="jt-glass-bar fixed bottom-3 left-1/2 z-40 grid h-[70px] w-[calc(100%-24px)] max-w-[680px] -translate-x-1/2 grid-cols-5 gap-1 p-1.5 sm:bottom-4"
        >
          {navItems.map((item) => {
            ...
            return (
              <Link
                ...
                className={cn(
                  "flex flex-col items-center justify-center rounded-xl text-[10px] font-bold transition sm:text-[11px]",
                  selected
                    ? "text-[var(--accent)]"
                    : "text-[#66717d] hover:bg-white/30 hover:text-[#1d1d1f]",
                )}
              >
                <span
                  className={cn(
                    "mb-0.5 flex h-7 w-8 items-center justify-center rounded-[10px]",
          ...
          <div className="group relative">
            <button
              ...
              className={cn(
                "flex h-full w-full flex-col items-center justify-center rounded-xl text-[10px] font-bold transition sm:text-[11px]",
                moreActive
                  ? "text-[var(--accent)]"
                  : "text-[#66717d] hover:bg-white/30 hover:text-[#1d1d1f]",
              )}
            >
              <span
                className={cn(
                  "mb-0.5 flex h-7 w-8 items-center justify-center rounded-[10px]",
                  moreActive && "bg-[var(--accent-light)]",
                )}
              >
            ...
            <div className="invisible absolute bottom-full right-0 w-64 pb-3 opacity-0 transition group-hover:visible group-hover:opacity-100 group-focus-within:visible group-focus-within:opacity-100">
              <div className="jt-floating-card grid gap-1 p-2" role="menu">
